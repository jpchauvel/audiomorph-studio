...
- Added `ModelDownloadManager` with strict required-model catalog, model*id validation (`[A-Za-z0-9*/-]`, no `..`), and containment guard to keep downloads under `<models_dir>/<repo_id>/`.
...
- window.**AUDIOMORPH_IPC** mocked in tests via page.addInitScript

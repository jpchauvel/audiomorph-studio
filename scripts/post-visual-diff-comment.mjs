...
const SNAPSHOTS_ROOT = path.join(REPO_ROOT, 'apps', 'renderer', 'tests', 'visual', '__snapshots__');
...
async function findExistingComment(repo, prNumber) {
  ...
  const { stdout } = await ghApi(['--paginate', `repos/${repo}/issues/${prNumber}/comments`]);
...
async function main() {
  ...
  try {
    ...
    console.warn(`visual-bot: comment API failed (non-fatal): ${err.stderr ?? err.message ?? err}`);
  }

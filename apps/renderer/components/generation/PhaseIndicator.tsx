...
export function PhaseIndicator() {
  ...
  return (
    <div className="w-full flex flex-col gap-4 mt-6 p-6 rounded-2xl bg-[var(--color-surface-2)] border border-[var(--color-border)]" ref={containerRef}>
            ...
            <div key={p.id} className="flex flex-col items-center gap-2 z-10">
              <div
                className={cn(
                  ...
                  isActive ? "bg-[var(--color-primary)] text-[var(--color-surface)] border-[var(--color-primary)] shadow-[0_0_15px_var(--color-primary)]" 
                  : isPast ? "bg-[var(--color-success)] text-[var(--color-surface)] border-[var(--color-success)]" 
                  : "bg-[var(--color-surface-3)] text-[var(--color-text-muted)] border-[var(--color-border)]"
                )}
              >
                {isPast ? '✓' : i + 1}
              </div>
              <span className={cn("text-xs font-medium", isActive ? "text-[var(--color-primary)]" : "text-[var(--color-text-muted)]")}>
                {p.label}
              </span>
            </div>
          )
      ...
      <div className="flex justify-between text-xs text-[var(--color-text-muted)] px-1 mt-2">
        ...
        <span>
          {etaS !== null && etaS > 0 ? `~${Math.ceil(etaS)}s remaining` : ''}
        </span>
      </div>

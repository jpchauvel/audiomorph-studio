...
export function ExportDialog({ open, onClose, jobId }: Props) {
  ...
  const apiBase = typeof window !== 'undefined' ? (window as any).__AUDIOMORPH_API_BASE__ ?? 'http://localhost:8000' : 'http://localhost:8000'
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent style={{ background: 'var(--color-surface-2)', border: '1px solid var(--color-border)' }}>
        <DialogHeader>
          <DialogTitle style={{ color: 'var(--color-text)' }}>Export Audio</DialogTitle>
        </DialogHeader>
        <div className="flex flex-col gap-4 py-2">
          <div>
            <label className="text-sm mb-1 block" style={{ color: 'var(--color-text-muted)' }}>Format</label>
          ...
          {format === 'mp3' && (
            <div>
              <label className="text-sm mb-1 block" style={{ color: 'var(--color-text-muted)' }}>Bitrate</label>

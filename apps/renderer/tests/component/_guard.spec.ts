...
test('CI guard — component config invariants', async ({ page }) => {
  ...
  expect(validMode, `AUDIOMORPH_TEST_MODE must be undefined or "1", got: ${JSON.stringify(mode)}`).toBe(true);
  ...
  const integrationLeak = await page.evaluate(() => {
    return (window as unknown as { __AUDIOMORPH_INTEGRATION_SETUP__?: unknown }).__AUDIOMORPH_INTEGRATION_SETUP__;
  });

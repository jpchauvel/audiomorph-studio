...
test.beforeEach(async ({ page }) => {
  ...
  await page.route('**/first-run/status', (route) =>
    route.fulfill({ json: { completed: false, missing_steps: ['pick_models_dir', 'download_models'] } })
  )
})
...
test('step progression: 1 → 2 → 3', async ({ page }) => {
  ...
  await page.route('**/models', (route) =>
    route.fulfill({ json: [{ id: 'm1', repo_id: 'HeartMuLa/Gen', name: 'HeartMuLaGen', size_gb: 5, state: 'missing' }] })
  )

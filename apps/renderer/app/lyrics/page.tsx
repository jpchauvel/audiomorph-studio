...
export default function LyricsPage() {
  ...
  const transcribe = async (file: File) => {
    ...
    try {
      ...
      if (!res.ok) { toast.error('Transcription failed'); setTranscribing(false); return }
  ...
  const cancel = async () => {
    if (!jobId) return
    esRef.current?.close()
    await fetch(`${API_BASE()}/lyrics/jobs/${jobId}`, { method: 'DELETE', headers: headers() }).catch(() => {})
    setTranscribing(false)
    setJobId(null)
  }
  ...
  return (
    <div className="max-w-2xl mx-auto p-8 flex flex-col gap-6" data-testid="lyrics-workspace">
      ...
      {segments.length > 0 && (
        <div className="flex flex-col gap-1 max-h-48 overflow-y-auto rounded-lg p-3" style={{ background: 'var(--color-surface-3)' }}>
          {segments.map((seg, i) => (
            <div key={i} className="flex gap-3 text-sm">
              <span style={{ color: 'var(--color-text-muted)', minWidth: 80 }}>{fmt(seg.start)} – {fmt(seg.end)}</span>
      ...
      <Button onClick={useInGeneration} disabled={!lyrics.trim()} data-testid="use-in-generation-btn">
        Use in Generation →
      </Button>
    </div>
  )

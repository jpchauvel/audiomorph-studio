...
function handleTyped<C extends IpcInvokeChannel>(
  channel: C,
  handler: (event: Electron.IpcMainInvokeEvent, payload: IpcInvokeMap[C]["in"]) => Promise<IpcInvokeMap[C]["out"]> | IpcInvokeMap[C]["out"],
): void {
...
function resolveAllowedRoots(): string[] {
  ...
  const sidecarTmp = process.env.AUDIOMORPH_SIDECAR_TMP_DIR ?? path.join(os.tmpdir(), "audiomorph-studio");
...
export function registerIpcBridge(options: RegisterIpcBridgeOptions = {}): void {
  ...
  handleTyped("api:request", async (_event, payload: ApiRequestInput): Promise<ApiRequestOutput> => {

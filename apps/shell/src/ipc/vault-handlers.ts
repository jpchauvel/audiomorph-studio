...
import type { VaultDeleteInput, VaultGetInput, VaultHasInput, VaultSetInput } from "@audiomorph/ipc-contracts";
...
export async function getSecretForSidecar(key: VaultKey, options: RegisterVaultHandlersOptions = {}): Promise<string | null> {
  const vault = getVault(options.vault);
  const audit = getAudit(options.auditLog);

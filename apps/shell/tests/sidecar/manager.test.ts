...
function setupHttpMocks(): void {
  mocks.httpGetMock.mockImplementation((_opts, cb?: (res: EventEmitter & { statusCode?: number; resume: () => void }) => void) => {
  ...
  mocks.httpRequestMock.mockImplementation((_opts, cb?: (res: EventEmitter & { resume: () => void }) => void) => {
...
describe("SidecarManager", () => {
  ...
  it("graceful shutdown deletes pid file when process exits on SIGTERM", async () => {
    ...
    const { SidecarManager } = await import("../../src/sidecar/manager");
    const manager = new SidecarManager({ userDataPath: "/tmp/audiomorph", logger: { log: vi.fn() }, tokenGenerator: () => "aterm" });
  ...
  it("force kills sidecar when process hangs after SIGTERM", async () => {
    ...
    const { SidecarManager } = await import("../../src/sidecar/manager");
    const manager = new SidecarManager({ userDataPath: "/tmp/audiomorph", logger: { log: vi.fn() }, tokenGenerator: () => "ahang" });

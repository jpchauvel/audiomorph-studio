...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    app,
    ipcMain: {
      handle: vi.fn((channel: string, cb: (event: any, payload: any) => Promise<unknown> | unknown) => {
...
describe("IPC bridge", () => {
  ...
  it("api:request proxies with X-Audiomorph-Token header and returns status/body", async () => {
    ...
    const out = (await handler?.({ sender: { send: vi.fn() } }, { method: "POST", path: "/jobs", body: { a: 1 } })) as {
      status: number;
      body: unknown;
    };
  ...
  it("api:cancel aborts in-flight api:request", async () => {
    ...
    const requestPromise = reqHandler({ sender: { send: vi.fn() } }, { method: "GET", path: "/healthz", requestId: "r-1" });
  ...
  it("fs:copyFile rejects path traversal outside allowlist", async () => {
    ...
    await expect(copyHandler({ sender: { send: vi.fn() } }, { src: "../../../etc/passwd", dst: "/tmp/userData/out.txt" })).rejects.toMatchObject({
  ...
  it("shell:openExternal enforces allowlist", async () => {
    ...
    await expect(handler({ sender: { send: vi.fn() } }, { url: "https://evil.example.com/phish" })).rejects.toMatchObject({
      code: "URL_NOT_ALLOWED",
    });

    await expect(handler({ sender: { send: vi.fn() } }, { url: "https://huggingface.co/spaces" })).resolves.toEqual({ ok: true });
